...
namespace Windows
{
    ...
    void ConnectionPanel()
    {
        ...
        if (ImGui::Button(id, ImVec2(175, 0)))
        {
            ime_single_field = smb_settings->server_ip;
            ResetImeCallbacks();
            ime_field_size = 16;
            ime_callback = SingleValueImeCallback;
            Dialog::initImeDialog(lang_strings[STR_SERVER], smb_settings->server_ip, 15, SwkbdType_Normal, 0, 0);
        ...
        if (ImGui::Button(id, ImVec2(100, 0)))
        {
            ...
            ime_field_size = 32;
            ime_callback = SingleValueImeCallback;
            Dialog::initImeDialog(lang_strings[STR_USERNAME], smb_settings->username, 32, SwkbdType_Normal, 0, 0);
            gui_mode = GUI_MODE_IME;
        }
        ImGui::SameLine();
        ...
        if (ImGui::Button(id, ImVec2(60, 0)))
        {
            ...
            ime_field_size = 24;
            ime_callback = SingleValueImeCallback;
            Dialog::initImeDialog(lang_strings[STR_PASSWORD], smb_settings->password, 24, SwkbdType_Normal, 0, 0);
            gui_mode = GUI_MODE_IME;
        }
        ImGui::SameLine();
